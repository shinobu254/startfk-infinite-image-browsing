...
from scripts.iib.tool import get_sd_startfk_conf, get_valid_img_dirs, sd_img_dirs, normalize_paths
...
def sd_startfk_paths_check(sd_startfk_config: str, relative_to_config: bool):
    conf = {}
    with open(sd_startfk_config, "r") as f:
        conf = json.loads(f.read())
    if relative_to_config:
        for dir in sd_img_dirs:
            if not os.path.isabs(conf[dir]):
                conf[dir] = os.path.normpath(
                    os.path.join(sd_startfk_config, "../", conf[dir])
                )
...
def do_update_image_index(sd_startfk_config: str, relative_to_config=False):
    dirs = get_valid_img_dirs(
        get_sd_startfk_conf(
            sd_startfk_config=sd_startfk_config,
            sd_startfk_path_relative_to_config=relative_to_config,
        )
    )
    if not len(dirs):
...
class AppUtils:
    def __init__(
        self,
        sd_startfk_config: Optional[str] = None,
        update_image_index: bool = False,
        extra_paths: List[str] = [],
        sd_startfk_path_relative_to_config=False,
        allow_cors=False,
        enable_shutdown=False,
        sd_startfk_dir: Optional[str] = None,
    ):
        """
        Parameter definitions can be found by running the `python app.py -h `command or by examining the setup_parser() function.
        """
        self.sd_startfk_config = sd_startfk_config
        self.update_image_index = update_image_index
        self.extra_paths = extra_paths
        self.sd_startfk_path_relative_to_config = sd_startfk_path_relative_to_config
        self.allow_cors = allow_cors
        self.enable_shutdown = enable_shutdown
        self.sd_startfk_dir = sd_startfk_dir
        if sd_startfk_dir:
            DataBase.path = os.path.join(
                sd_startfk_dir, "extensions/startfk-infinite-image-browsing/iib.db"
            )
            self.sd_startfk_config = os.path.join(sd_startfk_dir, "config.json")
            self.sd_startfk_path_relative_to_config = True
    ...
    def wrap_app(self, app: FastAPI) -> None:
        ...
        """
        sd_startfk_config = self.sd_startfk_config
        update_image_index = self.update_image_index
        extra_paths = self.extra_paths

        if sd_startfk_config:
            sd_startfk_paths_check(sd_startfk_config, self.sd_startfk_path_relative_to_config)
            if update_image_index:
                do_update_image_index(
                    sd_startfk_config, self.sd_startfk_path_relative_to_config
                )
        ...
        infinite_image_browsing_api(
            ...
            sd_startfk_path_relative_to_config=self.sd_startfk_path_relative_to_config,
...
def setup_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="A fast and powerful image browser for Stable Diffusion."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sd_startfk_config", type=str, default=None, help="The path to the config file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sd_startfk_path_relative_to_config",
        action="store_true",
        help="Use the file path of the sd_startfk_config file as the base for all relative paths provided within the sd_startfk_config file.",
    )
    parser.add_argument(
        "--allow_cors",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to the sd_startfk folder. When specified, the sd_startfk's configuration will be used and the extension must be installed within the sd_startfk. Data will be shared between the two.",
    )
    return parser
