...
from scripts.iib.tool import (
    ...
    is_img_created_by_comfyui_with_startfk_gen_info,
    is_valid_image_path,
    temp_path,
    read_sd_startfk_gen_info_from_image,
    get_formatted_date,
    is_win,
    cwd,
    locale,
...
def infinite_image_browsing_api(app: FastAPI, **kwargs):
    ...
    def get_img_search_dirs():
        try:
            return get_valid_img_dirs(get_sd_startfk_conf(**kwargs))
    ...
    def update_all_scanned_paths():
        ...
        if allowed_paths:
            sd_startfk_conf = get_sd_startfk_conf(**kwargs)
            path_config_key_map = {
                "save": "outdir_save",
            ...
            def path_map(path: str):
                ...
                if path in path_config_key_map:
                    return sd_startfk_conf.get(path_config_key_map.get(path))
                return path
    ...
    async def global_setting():
        ...
        try:
            ...
            print(e)
        return {
            "global_setting": get_sd_startfk_conf(**kwargs),
            "cwd": cwd,
            "is_win": is_win,
    ...
    async def image_geninfo(path: str):
        with Image.open(path) as img:
            if is_img_created_by_comfyui(img):
                if is_img_created_by_comfyui_with_startfk_gen_info(img):
                    return read_sd_startfk_gen_info_from_image(img, path)
                else:
                    try:                    
